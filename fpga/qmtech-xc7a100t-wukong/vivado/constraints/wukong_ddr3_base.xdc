...
# UART on the on-board CH340N at E3/F3 -- the default for every single-system
# Wukong build (JopConfig.wukongConsolePart).
#
# This routed to J11 -> Pico uart0 until 2026-08-23, because a second
# 1a86:7523 bridge on the host could not be told apart from the A-E115FB's.
# Those adapters now pass their serial numbers through, so both boards' CH340s
# are addressable at once (fpga/scripts/usb_serial_map) and the workaround is
# no longer needed.
#
# To put a build back on the Pico, do NOT edit this file -- regenerate with
# `uart=PICO_UART0`, which moves the config and the constraints together.
# J11 pinout, kept because that override needs it:
#
