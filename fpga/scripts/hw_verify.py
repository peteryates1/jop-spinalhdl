...
def stale_sources(image, src_dirs, limit=5, exts=(".java", ".properties")):
...
def main():
    ...
    # THE BITSTREAM MUST NOT BE OLDER THAN THE RTL EITHER (2026-09-14).
    # `make all` rebuilds the .sof; only `make program` regenerates the .rbf,
    # and find_bitstream PREFERS the .rbf for openFPGALoader. The CYC5000 spent
    # an afternoon looking like a software hang -- IO_GC_HALTED read 0 where an
    # identical board read 1, with byte-identical RTL and the same .jop --
    # because the register being polled did not exist in the three-day-old
    # bitstream on the chip. DoAll passed on it, which is what made it look like
    # a software bug rather than a stale artefact.
    stale_bits = stale_sources(bitstream, [os.path.join(cfg_dir, "rtl")],
                               exts=(".v", ".vhd"))
    if stale_bits:
        die("the bitstream is OLDER than the RTL it was built from -- this run\n"
            "  would test a chip that does not contain the design:\n"
            "    bitstream {}\n"
            "    newer     {}\n"
            "  rebuild it. Note `make all` may refresh only the .sof: the .rbf\n"
            "  is regenerated by the program target, and openFPGALoader reads\n"
            "  the .rbf."
            .format(os.path.relpath(bitstream, ROOT), "\n              ".join(stale_bits)))
