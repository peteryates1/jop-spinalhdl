...
public class Jopa {
	...
	private String objectName = "JumpTableData";
					...
					if (l.special=='=') {
						if (l.symVal==null) {
							...
							if (symMap.containsKey(l.symVal)) {
								...
								symMap.put(l.symVal, Integer.valueOf(l.intVal));
							}
						}
					} else if (l.special=='?') {
						if (symMap.containsKey(l.symVal)) {
							...
							symMap.put(l.symVal, Integer.valueOf(memcnt++));
...
//
					...
					if (l.instr.opdSize!=0) {
						...
						if (l.instr.name.equals("ldi")) {
							Integer i = Integer.valueOf(opVal);
							Integer addr;
							if (constMap.containsKey(i)) {
								addr = constMap.get(i);
							} else {
								addr = Integer.valueOf(constMap.size());
								constMap.put(i, addr);
								constList.add(i);
							}
							opVal = addr.intValue();
...
//			line += "\t[0..ff] : 00000000;\n";
	...
	/**
	 * Write Scala jump table for SpinalHDL.
	...
	private void writeScalaJumpTable(Map<Integer, Integer> bcToAddr,
		...
		FileWriter scala = new FileWriter(dstDir + objectName + ".scala");

		StringBuilder sb = new StringBuilder();

		// File header
		sb.append("/*\n");
		sb.append(" * " + objectName + ".scala\n");
		sb.append(" *\n");
		...
		sb.append("\n");
		sb.append("object " + objectName + " extends JumpTableSource {\n");
		sb.append("  /**\n");
		...
		for (int bc = 0; bc < 256; bc++) {
			...
			if (addr == null) {
				...
				addr = noim_addr;
				String bcName = JopInstr.name(bc);
				if (bcName != null && !bcName.isEmpty()) {
					mnemonic = bcName + ", unmapped -> sys_noim";
				} else {
		...
		System.out.println("Generated Scala jump table: " + dstDir + objectName + ".scala");
	}
...
*/
	public static void main(String args[]) {
		...
		if (args.length < 1) {
			System.out.println(
				"usage: java Jopa [-s srcDir] [-d dstDir] [-n ObjectName] filename");
			System.exit(-1);
		}
