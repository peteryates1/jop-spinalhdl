...
public class JopSim {
	...
	enum Access {
		CLINFO, MTAB, CONST, IFTAB, HANDLE, ALEN, MVB, FIELD, STATIC, ARRAY, INTERN;
		...
		void reset() { rdCnt = wrCnt = 0; }
	};

	static final int MAX_MEM = 1024*1024/4;
	static final int MAX_STACK = 64*1024;		// sim needs large stack (no spill to ext mem)
	static final int MEM_TEST_OFF = 256;
	static final int MAX_SCRATCHPAD = 256;
	...
	// trace level: 0=off, 1=method entry/exit, 2=+fields+null warnings, 3=full bytecode
	...
	public JopSim(String binaryFile, IOSimMin ioSim, int maxInstructions) {
		...
		if (ioSim.cpuId==0) {
			...
			// Use JopInstr cycle counts instead of WCETInstruction
			for (int i=0; i<256; ++i) {
				int j = JopInstr.cnt(i);
				if (j<=0) j = 80; // rough estimate for Java-implemented bytecodes
				bcTiming[i] = j;
			}
		}
	...
	void jjvmConst(int instr) {
		int idx = readOpd16u();
		int val = readMem(cp+idx, Access.CONST);
		stack[++sp] = val;
		invoke(jjp+(instr<<1));
	}
	...
	void invokesuper() throws JopSimRtsException {
		int idx = readOpd16u();
		int off = readMem(cp+idx, Access.CONST);
		int args = off & 0xff;
		off >>>= 8;
	...
	void invokeinterface() throws JopSimRtsException {
		...
		int vt = readMem(ref+1, Access.MVB);
		int it = readMem(vt-1, Access.CLINFO);
	...
	void invoke(int new_mp) {
		...
		// trace method entry
		if (traceLevel >= 1 && symbols != null) {
			String mName = symbols.methodContaining(new_mp);
			if (mName != null) {
				System.out.println("[TRACE] >> ENTER " + mName);
			}
		}
	}
	...
	void vreturn() {
		...
		// trace method exit
		if (traceLevel >= 1 && symbols != null) {
			String mName = symbols.methodContaining(mp);
			if (mName != null) {
				System.out.println("[TRACE] << EXIT  " + mName);
			}
		}
	...
	void waitCache(int hiddenCycles) {
		// no-op: timing not needed for debug simulator
	}

	void putstatic() {
		int addr = readOpd16u();
		if (traceLevel >= 2 && symbols != null) {
			String name = symbols.staticFieldName(addr);
			if (name != null) {
				System.out.println("[TRACE]   putstatic " + name + " = " + stack[sp]);
			}
		}
	...
	void getstatic() {
		int addr = readOpd16u();
		int val = readMem(addr, Access.STATIC);
		if (traceLevel >= 2 && symbols != null) {
			String name = symbols.staticFieldName(addr);
			if (name != null) {
				System.out.println("[TRACE]   getstatic " + name + " -> " + val);
			}
		}
		stack[++sp] = val;
	}
	...
	void checkNullHandle(int ref, String op) {
		if (traceLevel >= 2 && symbols != null && ref == 0) {
			String mName = symbols.methodContaining(mp);
			System.out.println("*** NULL HANDLE *** " + op + " in " +
				(mName != null ? mName : "mp=" + mp));
		}
	}
	...
	void interpret() {
		...
		if (traceLevel >= 3 && symbols != null) {
			String mName = symbols.methodContaining(mp);
			System.out.println("[TRACE]   " +
				(mName != null ? mName : "mp=" + mp) +
				" pc=" + (pc-1) + " " + JopInstr.name(instr));
		}
		...
		try {
			switch (instr) {
				...
				case 190 :		// arraylength
					ref = stack[sp--];
					checkNullHandle(ref, "arraylength");
					checkNullPointer(ref);
				...
				case 194 :		// monitorenter
					// Hardware microcode — handle inline (JVM.java has noim() stub)
					sp--; // pop object ref (not used in single-CPU sim)
					io.monEnter();
					break;
	...
	void stat() {
		...
		for (Access a : Access.values()) {
			...
			int rdPerc = (rdMemCnt + wrMemCnt) > 0 ? (int) ((double) a.rdCnt/(rdMemCnt + wrMemCnt)*1000) : 0;
			int wrPerc = (rdMemCnt + wrMemCnt) > 0 ? (int) ((double) a.wrCnt/(rdMemCnt + wrMemCnt)*1000) : 0;
	...
	public static int getArgs(String args[]) {
		...
		traceLevel = Integer.parseInt(System.getProperty("trace", "0"));
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < args.length; i++) {
			if ("-link".equals(args[i]) && i + 1 < args.length) {
				linkFile = args[++i];
			} else if (binaryFile == null) {
				binaryFile = args[i];
			} else {
				maxInstr = Integer.parseInt(args[i]);
			}
		}

		if (binaryFile == null) {
			System.out.println("usage: java JopSim [-link file.link.txt] file.jop [max instr]");
			System.exit(-1);
		}

		// Load symbols if link file provided
		JopSymbols symbols = null;
		if (linkFile != null) {
			symbols = new JopSymbols();
			symbols.load(linkFile);
		}
		...
		for (int i=0; i<nrCpus; ++i) {
			...
			io.setCpuId(i);
			js[i] = new JopSim(binaryFile, io, maxInstr);
			js[i].symbols = symbols;
		}
