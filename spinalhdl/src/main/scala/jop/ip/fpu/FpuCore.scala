...
case class FpuCore( portCount : Int, p : FpuParameter) extends Component{
  ...
  val scheduler = for(portId <- 0 until portCount;
    ...
    // JOP patch: pending must track ALL commands (not just useRd) so that
    // non-useRd commits (STORE, FMV_X_W, F2I, CMP, FCLASS) can fire.
    // In VexRiscv's pipelined CPU, the pipeline depth naturally keeps
    // pending > 0 when these commits arrive. In JOP's FSM adapter,
    // commands are issued sequentially, so pending drains to 0 before
    // the STORE commit, causing a deadlock.
    when(output.fire){
