...
/**
 * Test Bench Wrapper for DecodeStage (CocoTB Compatible)
 *
 * This wrapper provides the same interface as decode_tb.vhd for CocoTB testing.
 * Uses noIoPrefix() to remove the io_ prefix from port names.
 *
 * Interface matches decode_tb.vhd exactly for seamless CocoTB verification.
 */
case class DecodeStageTb(
  iWidth: Int = 10,
  ramWidth: Int = 8
) extends Component {
  ...
  // Clock and reset (directly wired to component's implicit clock)
  val clk   = in Bool()
  val reset = in Bool()

  // Inputs
  val instr = in Bits(iWidth bits)
  val zf    = in Bool()
  val nf    = in Bool()
  val eq    = in Bool()
  val lt    = in Bool()
  ...
  // Memory Control Outputs (mem_in record fields exposed individually)
  val mem_in_bcopd     = out Bits(16 bits)
  val mem_in_rd        = out Bool()
  val mem_in_wr        = out Bool()
  val mem_in_addr_wr   = out Bool()
  val mem_in_bc_rd     = out Bool()
  val mem_in_iaload    = out Bool()
  val mem_in_iastore   = out Bool()
  val mem_in_stidx     = out Bool()
  val mem_in_getfield  = out Bool()
  val mem_in_putfield  = out Bool()
  val mem_in_getstatic = out Bool()
  val mem_in_putstatic = out Bool()
  val mem_in_putref    = out Bool()
  val mem_in_rdc       = out Bool()
  val mem_in_rdf       = out Bool()
  val mem_in_wrf       = out Bool()
  val mem_in_copy      = out Bool()
  val mem_in_cinval    = out Bool()
  val mem_in_atmstart  = out Bool()
  val mem_in_atmend    = out Bool()

  // MMU instruction select
  val mmu_instr = out Bits(4 bits)
  val mul_wr    = out Bool()
  val wr_dly    = out Bool()

  // Direct RAM address
  val dir = out Bits(ramWidth bits)

  // ALU Control Signals
  val sel_sub  = out Bool()
  val sel_amux = out Bool()
  val ena_a    = out Bool()
  val sel_bmux = out Bool()
  val sel_log  = out Bits(2 bits)
  val sel_shf  = out Bits(2 bits)
  val sel_lmux = out Bits(3 bits)
  val sel_imux = out Bits(2 bits)
  val sel_rmux = out Bits(2 bits)
  val sel_smux = out Bits(2 bits)
  val sel_mmux = out Bool()
  val sel_rda  = out Bits(3 bits)
  val sel_wra  = out Bits(3 bits)
  val wr_ena   = out Bool()
  val ena_b    = out Bool()
  val ena_vp   = out Bool()
  val ena_jpc  = out Bool()
  val ena_ar   = out Bool()

  // Create a new clock domain using the explicit clk and reset signals
  val decodeClockDomain = ClockDomain(
    clock = clk,
    reset = reset,
  ...
  // Instantiate the decode logic in the explicit clock domain
  val decodeArea = new ClockingArea(decodeClockDomain) {
    val config = DecodeConfig(iWidth, ramWidth)
    ...
    // MMU Instruction Output (combinational)
    // ========================================================================
    mmu_instr := ir(config.mmuWidth - 1 downto 0)
    ...
    switch(ir(9 downto 6)) {
      is(B"4'b0000") { isPop := True }
      is(B"4'b0001") { isPop := True }
      is(B"4'b0010") { isPush := True }
      is(B"4'b0011") { isPush := True }
      is(B"4'b0100") { /* NOP class */ }
      is(B"4'b0101") { /* null */ }
      is(B"4'b0110") { isPop := True }
      is(B"4'b0111") { isPop := True }
      default { /* JMP class and others */ }
    }
    ...
    wr_ena := False
    when(isPush ||
         ir(9 downto 5) === B"5'b00001" ||
         ir(9 downto 3) === B"7'b0000010") {
      wr_ena := True
    }

    // Immediate Mux Select
    sel_imux := ir(1 downto 0)

    // Direct RAM Address
    dir := B(0, ramWidth - 5 bits) ## ir(4 downto 0)
    when(ir(9 downto 5) === B"5'b00110") {
      dir := B(1, ramWidth - 5 bits) ## ir(4 downto 0)
    }

    // Read Address Mux Select
    sel_rda := B"3'b110"
    when(ir(9 downto 3) === B"7'b0011101") { sel_rda := ir(2 downto 0) }
    when(ir(9 downto 5) === B"5'b00101") { sel_rda := B"3'b111" }
    when(ir(9 downto 5) === B"5'b00110") { sel_rda := B"3'b111" }

    // Write Address Mux Select
    sel_wra := B"3'b110"
    when(ir(9 downto 3) === B"7'b0000010") { sel_wra := ir(2 downto 0) }
    when(ir(9 downto 5) === B"5'b00001") { sel_wra := B"3'b111" }

    // Stack Pointer Mux Select
    sel_smux := B"2'b00"
    when(isPop) { sel_smux := B"2'b01" }
    when(isPush) { sel_smux := B"2'b10" }
    when(ir === B"10'b0000011011") { sel_smux := B"2'b11" }

    // ========================================================================
    // Registered Branch/Jump Decode
    // ========================================================================
    val brReg  = Reg(Bool()) init(False)
    val jmpReg = Reg(Bool()) init(False)
    ...
    when((ir(9 downto 6) === B"4'b0110" && zf) ||
         (ir(9 downto 6) === B"4'b0111" && !zf)) {
      brReg := True
    }
    ...
    // Logic Operation Select
    selLogReg := B"2'b00"
    when(ir(9 downto 2) === B"8'b00000000") {
      selLogReg := ir(1 downto 0)
    }
    ...
    // Instruction-Specific ALU Control
    switch(ir) {
      is(B"10'b0000000000") { /* pop */ }
      is(B"10'b0000000001") { /* and */ }
      is(B"10'b0000000010") { /* or */ }
      is(B"10'b0000000011") { /* xor */ }
      is(B"10'b0000000100") {
        selSubReg := False
        selAmuxReg := False
      }
      ...
      is(B"10'b0000010100") { /* st */ }
      is(B"10'b0000010101") { /* stmi */ }
      is(B"10'b0000011000") { enaVpReg := True }
      is(B"10'b0000011001") { enaJpcReg := True }
      is(B"10'b0000011010") { enaArReg := True }
      is(B"10'b0000011011") { /* stsp */ }
      is(B"10'b0000011100") { /* ushr */ }
      is(B"10'b0000011101") { /* shl */ }
      is(B"10'b0000011110") { /* shr */ }
      ...
      is(B"10'b0011110110") { /* ld_opd_16u */ }
      is(B"10'b0011110111") { /* ld_opd_16s */ }
      is(B"10'b0011111000") { enaAReg := False }
      is(B"10'b0100000000") { enaAReg := False }
      is(B"10'b0100000001") { enaAReg := False }
      is(B"10'b0100000010") { enaAReg := False }
      is(B"10'b0100010000") { enaAReg := False }
      is(B"10'b0100010001") { enaAReg := False }
      is(B"10'b0100010010") { enaAReg := False }
      is(B"10'b0100010011") { enaAReg := False }
      default { /* use defaults */ }
    }

    when(ir(9)) { enaAReg := False }

    // Load Mux Select
    selLmuxReg := B"3'b000"
    when(ir(9 downto 2) === B"8'b00000111") { selLmuxReg := B"3'b001" }
    when(ir(9 downto 5) === B"5'b00101") { selLmuxReg := B"3'b010" }
    when(ir(9 downto 5) === B"5'b00110") { selLmuxReg := B"3'b010" }
    when(ir(9 downto 3) === B"7'b0011101") { selLmuxReg := B"3'b010" }
    when(ir(9 downto 2) === B"8'b00111101") { selLmuxReg := B"3'b011" }
    when(ir(9 downto 3) === B"7'b0011100") { selLmuxReg := B"3'b100" }
    when(ir(9 downto 2) === B"8'b00111100") { selLmuxReg := B"3'b101" }
    ...
    // Enable B
    enaBReg := True
    when(!isPush && !isPop) { enaBReg := False }
    ...
    // ========================================================================
    // Registered MMU/Memory Control Decode
    // ========================================================================
    val memRdReg        = Reg(Bool()) init(False)
    val memWrReg        = Reg(Bool()) init(False)
    val memAddrWrReg    = Reg(Bool()) init(False)
    ...
    mem_in_rd := memRdReg
    mem_in_wr := memWrReg
    mem_in_addr_wr := memAddrWrReg
    mem_in_bc_rd := memBcRdReg
    mem_in_stidx := memStidxReg
    mem_in_iaload := memIaloadReg
    mem_in_iastore := memIastoreReg
    mem_in_getfield := memGetfieldReg
    mem_in_putfield := memPutfieldReg
    mem_in_getstatic := memGetstaticReg
    mem_in_putstatic := memPutstaticReg
    mem_in_putref := memPutrefReg
    mem_in_rdc := memRdcReg
    mem_in_rdf := memRdfReg
    mem_in_wrf := memWrfReg
    mem_in_copy := memCopyReg
    mem_in_cinval := memCinvalReg
    mem_in_atmstart := memAtmstartReg
    mem_in_atmend := memAtmendReg
    mul_wr := mulWrReg
    wr_dly := wrDlyReg
 ...
 */
object DecodeStageTbVhdl extends App {
  ...
  // Generate the testbench wrapper (matches decode_tb interface)
  config.generateVhdl(DecodeStageTb(iWidth = 10, ramWidth = 8))
    .printPruned()

  println("DecodeStageTb VHDL generated at: generated/vhdl/DecodeStageTb.vhd")
}
