 ...
 */
class SdramExerciserClkWiz extends BlackBox {
  val io = new Bundle {
    val resetn        = in Bool()
    val clk_in        = in Bool()
...
/**
 * SDRAM Exerciser FPGA Top for QMTECH XC7A100T Wukong V3
 *
...
case class SdramExerciserWukongTop(md: MemoryDevice) extends Component {
  ...
  // ClkWiz: 50 MHz -> 100 MHz system + 100 MHz -108° SDRAM clock
  ...
  val mainArea = new ClockingArea(mainClockDomain) {
    ...
    // LEDs (active HIGH on Wukong)
    val ledReg = Reg(Bits(2 bits)) init (0)
    io.led := ledReg

    // J10 diagnostic LEDs: show state machine encoding for debug
    io.j10_led := state.asBits.resized
