...
10. **[#113](#item-113)** — `cold-check` covers 3 boards of 12, and the primary board is not one of them
11. **[#112](#item-112)** — `ConstraintDriftTest` covers 2 presets of 46 — the strongest check in the tree, at 4 % coverage
12. **[#82](#item-82)** — Flash boot has been unbuildable since 2026-03-13; a hardware-verified capability deleted as collateral
13. **[#68](#item-68)** — Ethernet links at 1 Gbps but no packets move
14. **[#65](#item-65)** — Both SD exercisers fail on hardware, and it is not the conversion
15. **[#84](#item-84)** — No MAX1000 configuration fits the 10M08 — single-core overflows it by a third
16. **[#73](#item-73)** — `ep4cgx150DbVgaDma` misses timing by −1.011 ns
17. **[#54](#item-54)** — Statics are Kfl's largest stall category (41 %) and no cache touches them. Count the accesses before designing anything
18. **[#55](#item-55)** — The core stalls on writes whose result it never uses — `idle/direct`, 39 % of Kfl stall. Needs read-after-write forwarding and an SMP story
19. **[#37](#item-37)** — The method cache dominates real memory traffic — 62 % of DoApp's BMB transactions, and [50](#item-50) confirms it in TIME on real memory: bytecode fill is 47-63 % of stall on Kfl and UdpIp
20. **[#64](#item-64)** — `GcStressTest` loses **~0.42 bytes/round** on three boards and three memory systems. Deterministic, so a defect, not drift — and TWO candidates are now RULED OUT by measurement: [item 131](#item-131)'s dropped card marks and [item 141](#item-141)'s stray barrier writes
21. **[#4](#item-4)** — Copy phase — 79-82% of the minor pause and the dominant remaining term
22. **[#39](#item-39)** — The L2 hit path is serial — 3 cycles per hit, 58-61 % of the DRAM access interval. **[50](#item-50) raises the priority of this**: bytecode fill is a sequential burst and improved only 3 % with a 32 KB L2 in front of DDR3, which is what a 3-cycle hit would predict
23. **[#44](#item-44)** — The compute floor C is per-configuration; re-measure it before trusting any per-operation cost
24. **[#45](#item-45)** — ONE unidentified register is read before it is written; the other ~401 look benign
25. **[#32](#item-32)** — UART corruption on seed 871203250 — no longer reachable, pin removed; cause never found
26. **[#5](#item-5)** — The BMB arbiter sets the clock ceiling — FREQUENCY, not core count
27. **[#31](#item-31)** — The BMB arbiter caps TIMING CLOSURE on both FPGA families (not throughput — see 2026-08-18 note)
28. **[#41](#item-41)** — Neither 8-core DRAM build closes timing, MSHRs or not
29. **[#70](#item-70)** — The UART baud is stated three times; console.mk now refuses an unknown one, but the i5 still hardcodes its own
30. **[#67](#item-67)** — `ep4cgx150DbFull` has `useStackCache` off; gated on item 14
31. **[#75](#item-75)** — `ep4cgx150HwMath` is a byte-identical duplicate of `ep4cgx150Serial`
32. **[#104](#item-104)** — The generators restate board facts as literals: one board's PLL shape, a device-wide I/O standard, a clock port spelled three ways
33. **[#105](#item-105)** — Assembly navigation uses `boards.head` where it means `fpgaBoard`; safe only because every composite lists the core board first
34. **[#106](#item-106)** — The device map is keyed by raw strings in three places — the surviving family of the `"eth"`/`"ethernet"` bug
35. **[#107](#item-107)** — `alchitry-au`'s `bitstream` has no prerequisite on `project` — racy under `make -j`
36. **[#3](#item-3)** — Sixteen presets still run classic GC. Safe but slow
37. **[#53](#item-53)** — 4-core Wukong takes `15/6` + `double:java` (64 blocks, DoAll 66/66, 68.5 % LUT). **The preset still does not build at defaults** — threshold needs the 8/12-core data
38. **[#52](#item-52)** — The Java tools hold hand-copied duplicates of the hardware config. Generate them from the preset instead
39. **[#17](#item-17)** — `needs*Compute` predicates understate compute-unit reachability
40. **[#18](#item-18)** — Software/microcode fallback coverage is uneven — 18 of 32 configurables
41. **[#19](#item-19)** — Write the missing `_sw` microcode handlers
42. **[#20](#item-20)** — Decide whether the double group gets microcode at all
43. **[#27](#item-27)** — The `aastore` type check's cost was never measured
44. **[#12](#item-12)** — `LongComputeUnitConfig` has no enable flag for its base 64-bit ALU
45. **[#7](#item-7)** — Root-scan floor: 2.2 / 4.7 / 8.5 ms across SDR / DDR3 / DDR2
46. **[#8](#item-8)** — XC7A100T timing margin is +0.001 ns — one bad run in seven
47. **[#14](#item-14)** — Stack cache SDRAM integration — 3-bank rotation verified in BRAM, needs per-core regions
48. **[#40](#item-40)** — A leaner MSHR entry — each holds a full cache line of write data a read miss never uses
49. **[#42](#item-42)** — Secondary-hit merging is not implemented — a request to a line being filled replays
50. **[#21](#item-21)** — Colorlight i5 is EBR-bound in BRAM-only builds, not logic-bound
51. **[#11](#item-11)** — Application benchmark exists (`java/apps/JbeBench`) — remaining questions it should answer
52. **[#13](#item-13)** — `java/apps/Small` `make clean` deletes `HelloWorld.jop`
53. **[#56](#item-56)** — WBNI: derive the hardware config from the application. **JOPizer static profile DONE**; the remaining bulk is a measurement FRAMEWORK (preferably Java) across the hardware set
54. **[#58](#item-58)** — `source` inside an XDC is silently ignored — SDRAM IOB packing and Ethernet GMII constraints have never been applied
55. **[#117](#item-117)** — Nothing prevents an eighth preset that no flow selects
56. **[#115](#item-115)** — Every simulation reports `Elaboration failed (2 errors)` and then succeeds; pre-existing, deterministic, unexplained
57. **[#108](#item-108)** — README's 16-core claim rests on resource figures README itself withdrew as undated
58. **[#100](#item-100)** — The EP4CGX150 cable reads 10/10 since the 2026-08-31 swap and blocks nothing. What is left is an unresolved confound: the swap changed the cable AND re-seated both plugs. **The proposed resolution — put the Pico back on that board — is BLOCKED: that Pico has since failed ([#148](#item-148))**
59. **[#63](#item-63)** — One unexplained Wukong SDR startup crash in six runs; not reproduced, cause unknown
60. **[#62](#item-62)** — `JopFloatCuBramSim` reads a `floatcu` microcode variant that has never been generated, so it has never run
61. **[#142](#item-142)** — The linked image's `<clinit>` ORDER is decided by hash iteration order in `OldClinitOrder.findOrder()`; any classpath change reshuffles it, and correctness then rests on a dependency analysis that misses `invokeinterface`
62. **[#143](#item-143)** — `setsid` + process-group kill leaks the downloader holding the serial port; the leftover reader then looks exactly like dead hardware
63. **[#148](#item-148)** — The A-E115FB is powered off after its Pico blaster failed, so DDR2 has NO hardware coverage; it also blocks item 100's confound experiment, which needs that blaster
...
- **[158](#item-158)** — ~~Stop-the-world has no acknowledgement~~ — FIXED 2026-09-14: `IO_GC_HALTED` + a grant rule; haltLeak 5213 → 0
...
### ~~Item 158~~ — "stop the world" is a request with no acknowledgement, and the lock-owner exemption guarantees it is sometimes not honoured — **FIXED 2026-09-14**
...
**FIXED 2026-09-14. `haltLeak 5213 → 0`, with the vehicle still in place.**

Guard: `IhluGcGrantTest` (6 cases) + `CmpSyncFormal` (2 properties).

| | before | after |
|---|---:|---:|
| `haltLeak` (cycles a mutator ran inside the halt) | **5213** | **0** |
| `haltWait` (iterations the collector waited) | — | **274** |

**The pair is the result, not the zero.** `haltWait 274` says the collector
genuinely blocked while an exempt owner drained. Had it read 0 the halt would
have been trivially satisfied and `haltLeak 0` would mean nothing — the same
vacuous-zero shape [item 157](#item-157) existed to eliminate.

**1. The acknowledgement.** `othersHalted` — AND of the other cores' `halted`,
computed in `CmpSync`/`Ihlu`, exposed as bit 0 of `IO_GC_HALTED` (the free READ
direction of `IO_INTMASK`, the trick `IO_ROOT_DATA` uses on `IO_GC_HALT`).
`GC.minorGc` now spins on it after asserting `IO_GC_HALT` and before touching
the heap. Deliberately unbounded: a bounded wait that gave up and proceeded
would restore the old behaviour silently, on the rare path, which is the worst
of both.

**2. The grant rule.** While any core asserts `gcHalt`, `Ihlu` services a
request only if it is an unlock, or comes from a core that already owns
something, or comes from the halt requester. A core owning nothing keeps
`hasPending` high, which is `lockWait`, which **halts it** — so refusing to
admit does not leave it spinning inside the stop-the-world, it parks it, which
is what the halt wanted. The request is serviced when the halt lifts.

**"Already owns something" is load-bearing, and dropping it deadlocks the
machine.** A nested `synchronized` needs a *new* slot; refuse that and the owner
never reaches its monitorexit, never leaves the exempt set, and the collector —
now waiting — waits forever. `IhluGcGrantTest` asserts both directions, because
the rule fails in two ways that look nothing alike from outside: granting too
much is silent corruption, granting too little is a cluster-wide hang.

The sim was PROVED RED on the two rule cases with all four controls passing:

```
FAIL: core 3 owned NOTHING and was granted a new lock during a gcHalt;
      it is now exempt from the halt it walked into.
FAIL: core 3's blocked request must leave it HALTED (hasPending -> lockWait)
```

**Timing**: +0.397 ns on the 8-core EP4CGX150, MET — better than the +0.167 the
item 157 counter left, and comfortably inside the fitter noise band this build
has shown all session (+0.35 … +0.45 across five same-tree builds).

**What this does NOT claim.** `lost 0` was already true before the fix: this
workload never lost a reference through the hole. The fix closes a
demonstrated-open window, and the evidence for it is `haltLeak` and `haltWait`,
not a failure that stopped happening. Make `mutatorTick` real first, or this lands unfalsifiable.
...
they are written**, by running them red against the unfixed code, and re-proved
when something makes them suspect. That is a judgement, and automating it would
be the nightmare it sounds like.

What this item is about is narrower: **a check whose output cannot distinguish
"I looked and found nothing wrong" from "I did not look".**

**Three things went wrong, and only the first is about the test itself.**

**(a) Guards that could not fail** — 2026-08-30/31, all three caught by hand:

- `check-status-index.sh` reported *"priority list: 0 entries, none closed"* —
  passing because the extraction regex matched nothing.
- `check-console-baud.sh` first asserted a non-zero exit, which the unfixed tree
  also produced (`No rule to make target 'require-baud'`), then a `grep -i baud`
  which matched that very message.
- `GeneratedConstraintsTest` asserted an invariant that was simply wrong.

**(b) Checks that reported success without EXECUTING** — 2026-09-03, four in a
day, and the worse shape because nothing about the output looks wrong:

- The pre-push simulation gate, twice: `sbt -J-Xmx4g` and `sbt --no-colors` are
  CI-only flags this sbt rejects, and the first version read `tail`'s exit
  status through a pipe rather than sbt's. It announced *"rc=0, zero NO DRIVER"*
  for three simulations that never started — `NO DRIVER` was zero because
  nothing had elaborated.
- `quartus_pgm` is not on the interactive PATH, so five retries against a dead
  JTAG chain printed five blanks that read as five clean detections.
- CR line endings: `grep -cE ' ok$'` returned 0 where the answer was 67, i.e.
  "the board failed every test".
- The `grep` wrapper ([item 135](#item-135)).

**(c) A stale artefact, so the check ran against the wrong thing** —
[item 140](#item-140): a linker change that never reached the image, caught only
because the result was *impossible*.

**What actually works, and none of it is recursive.**

1. **Assert a structural invariant, not a tally.** A count is only evidence to
   a reader who already knows what to expect — and `priority list: 1 entries` is
   a perfectly legitimate value, so the count could never have flagged the
   2026-09-03 renumbering on its own. `1..N contiguous with no gaps or repeats`
   needs no expectation: it is true or it is not. That check is now in
   `check-status-index.sh` and catches the exact corruption that prompted it.
2. **Never accept an exit status as evidence.** Assert on content — a test
   tally, a distinctive phrase, a line count. Every one of (b) is this.
3. **Verify against the ARTEFACT, not the build.** `command grep` the emitted
   `.jop`, read the elaborated Verilog, diff the generated constraint file. A
   build that exits 0 has not said the change took effect ([item 140](#item-140)).
4. **Record red-before-green where it is done.** The discipline is human and
   should stay human; what is missing is durability. "Verified red against the
...
**CLOSED 2026-09-04, with the residue moved rather than carried.** What remains
— proving a test red when it is written, re-proving one that becomes suspect —
is a standing practice, and a backlog that carries standing practices stops
being a list of things needing attention and becomes a list of things one ought
to be doing. Those are different documents. The practice now lives in
[docs/testing-discipline.md](testing-discipline.md), and is echoed at the point
of use: every guard in `.github/scripts/` carries a `DISCIPLINE:` header naming
which practice it follows and a `PROVED RED` line recording the defect it was
demonstrated against, and the four tests written to this pattern
(`ArrayCacheBoundsTest`, `CardTableTest`, `JopCardClearStallSim`,
`jvm/StringLiteral`) say the same in their own headers.

That echo is the durable half of practice 4: a guard whose header states how it
was proved red can be re-proved by the next person without archaeology, and one
that carries no such line is visibly untrusted.
...
**COMPLETE 2026-09-02.** All four ran. **Nine boundaries, nine with findings** —
the method has still not produced a clean one.

The last two returned the most serious defect of the whole series:
[item 136](#item-136), where a String literal has no valid `OFF_TYPE`, so
`instanceof String` is false, `String[]` stores throw, and
`heapString.equals(literal)` is silently wrong. It was reproduced with a
control, confirmed against the shipped image, and traced to a fix from
2026-08-06 that was validated with "ArrayCastTest 15/15 and DoAll 66/66" —
both true, both uninformative, because JOPizer interns literals and every
`String.equals` in the suite is literal-vs-literal and short-circuits on
`this == anObject`. Also [item 137](#item-137) and [item 138](#item-138).

B6 additionally found [item 135](#item-135) — the shell's `grep` silently
returns nothing for a common invocation — which is a hazard aimed squarely at
this method's central rule.
...
### Item 129 — ~~`arraylength` has no null check, so `null.length` reads address 1~~ — FIXED
...
> **FIXED 2026-09-05.** One condition in `BmbMemoryController`, no microcode
> change:
>
> ```scala
...
> ```
>
> **Why `== 1` and not `== 0`:** the microcode has already added the handle
...
> AND two cycles shorter on the hot path of every array loop. The machine then
> did not boot at all: empty UART after 60 M cycles. `rdf` is REGISTERED in
> `DecodeStage`, so it arrives a cycle after `STMRAF` decodes; with `ldi 1; add`
> ahead of it `io.aout` has settled, but with `stmraf` as the bytecode's first
> instruction it is no longer the arrayref. Isolated by three controlled runs
> (RTL+microcode → dead; microcode only → boots then throws; null check only →
> full boot). Recorded so it is not retried.
>
> **It costs frequency on the ECP5, measured against a control build.** The i5
> fell 53.93 → 46.18 MHz; hoisting the comparator out of the IDLE priority chain
> so synthesis evaluates it beside `aoutIsIo` recovered it to **48.29 MHz**. A
> control build at HEAD without the fix reproduced 53.93 exactly, so this is the
> change and not nextpnr variance — worth stating, because the same 7 MHz swing
> would have been dismissed as noise without the control.
>
> | board | before | after |
> |---|---|---|
> | EP4CGX150 | 85.65 MHz, 11,269 LE | **86.83 MHz**, 11,333 LE |
> | Wukong | WNS +0.315 ns | **WNS +0.407 ns** |
> | Colorlight i5 | 53.93 MHz | **48.29 MHz** (PASS at 40) |
>
> Accepted rather than made conditional: the i5 is the only board affected and
> keeps 8.29 MHz of margin, while the alternative — a microcode null test —
> costs ~6 cycles on EVERY array loop on EVERY board, and making the check
> per-config would give up the guarantee that `null.length` throws everywhere.
>
> **Validated:** `sbt test` 666/666; `JopJvmTestsBramSim`,
> `JopJvmTestsMcFallbackSim` and `JopDcuCacheSim` all 0 MISS; DoAll **68/68 on
> EP4CGX150, Wukong and Colorlight i5**, with `Array ok` and the
> `MISS: arraylength-NPE` line gone from every run. `jvm/Array.java`'s assertion
> is re-enabled and was proved red against the unfixed RTL first.
...
**The two halves.** `StringInfo.java:113-121` emits a link-time literal as a
String handle and a `char[]` handle packed together, so word `+3` of the String
is the ARRAY LENGTH:

```
+0  addr+4          String handle -> first field
+1  strcli.methodsAddress    String mtab
+2  addr+5          char[] handle -> first element
+3  string.length()          "array length in the handle"
+4  addr+2          the `value` field
```

`JVM.java:808` (and `:852`) reads `Native.rdMem(objref + GC.OFF_TYPE)` with
`OFF_TYPE = 3` and `IS_OBJ = 0`. For any non-empty literal that word is the
length, so `type != IS_OBJ` and the reference is diverted to
`JVMHelp.arrayCastOk`, which returns false for a non-array target class whose
`CLASS_SUPER > 0` — and `java.lang.String`'s super is `Object`, non-zero.

**Confirmed in `DoAll.jop:44319-44324`**, the shipped image, read directly
rather than inferred:

```
        //      39122   "Format.parseObject(S..."
        39126,  //      String handle points to the first field
        64657,  //       pointer to String mtab
        39127,  //      char[] handle points to the first element
        33,     // array length in the handle          <-- word 39125 = OFF_TYPE
        39124,  //      char ref. points to char[] handle
```

**Reproduced on `JopSim`**, with the control alongside:

```
A instanceof-literal:no     <- "abcd" instanceof String == FALSE
B instanceof-heap:yes       <- the control: a heap String works
C lit.equals(heap):yes
D heap.equals(lit):no       <- built.equals("abcd") FALSE for equal strings
E (String)literal:CCE
F String[]<-literal:THROW   <- sa[0] = "abcd" throws ArrayStoreException
G Object[]<-literal:ok
H String[]<-heap:ok
```

**Consequences in ordinary code.** `String[] a; a[0] = "x";` throws
ArrayStoreException. `(String) map.get(k)` throws ClassCastException. Worst,
`String.equals(Object)` returns **false** whenever the argument is a literal and
the receiver was built at run time — which is the comparison every
`Hashtable`/`HashMap` lookup and every `Vector.contains` performs. Silent wrong
answer, not an exception.

**The regression, and the irony.** `f064a8b` (2026-08-06) — *"jvm: stop
checkcast and instanceof walking garbage when given an array"* — fixed a real
bug by adding this `OFF_TYPE` check. Before it, `f_checkcast` went straight to
`rdMem(objref + OFF_MTAB_ALEN) - CLASS_HEADR`, which is correct for a literal.
The fix for arrays broke literals, because a literal is the one object whose
`OFF_TYPE` word belongs to something else.

**Why nothing caught it, exactly.** `f064a8b` was validated with "ArrayCastTest
15/15 and DoAll 66/66". Every `String.equals` in `java/apps/JvmTests/src/` is
literal-vs-literal — `"a".equals(v.elementAt(0))`, `exc.getMessage()
.equals("ok")` — and JOPizer INTERNS literals, so `this == anObject`
short-circuits before the `instanceof` ever runs. The suite has **zero**
coverage of the path. This is [item 111](#item-111)'s "nothing measures whether
...
The fix itself is one constant in `StringInfo.java`: `STR_OBJ_LEN` from
`2+2+1` to `4+2+1`, unpacking the String handle so word +3 is a real
`OFF_TYPE = IS_OBJ` instead of the char[]'s length. `ldc` still pushes `addr+0`
and `JopClassInfo.java:534` needs no change. **Every variant tried makes
`StringLiteral` pass**, and the String half alone is sufficient — widening the
char[] handle too is not needed.

| variant | image words | `StringLiteral` | `TextFormatTest` |
|---|---|---|---|
| baseline, defect present | 74061 | MISS x4 | **ok** |
| **padding control** — old layout, 4 words of trailing pad | 76195 | MISS x4 | **ok** |
| fix `4+4+1` | 76195 | **ok** | `noim` |
| fix `4+2+1`, String handle only | ~75k | **ok** | silent stop |
| fix `4+4+1` + non-zero `OFF_SPACE` | 76195 | **ok** | `noim` |

**The padding control is the load-bearing row.** Identical image size and
identical addresses to the fix, with literal semantics byte-for-byte unchanged —
and `TextFormatTest` passes. That eliminates image size, address shift, cycle
budget and heap pressure in a single run, all four of which had been plausible
and two of which had already been guessed wrong.

**What survives:** `TextFormatTest` breaks in exactly the variants where the
char[] handle leaves `literal+2` and the characters leave `literal+5`. `noim`
dies before printing its own mp/pc/bytecode diagnostic, so the stack is already
corrupt by then — it is a symptom, not the cause. Ruled out along the way:
heap pressure (doubling BRAM changes nothing), cycle budget (50 % more cycles
gives byte-identical output), image size and addresses (the control),
`OFF_SPACE` going from an accidental non-zero to 0 (restoring non-zero changes
nothing), `String` needing more than one field word (it has exactly one),
...
**ROOT CAUSE FOUND 2026-09-03, and it is a PRE-EXISTING defect that this fix
merely unmasks.**

The reproducer pinned the write to one statement. Dumping the literal's words
around each step of `new DecimalFormat("0")`:

```
W1 lit@25090 w=25096,31297,0,0,25097, 0,25094
   [c0 +5=0][c1 +5=0][a0 +5=0][a1 +5=0][a2 +5=-1][c2 +5=-1]
W2 lit@25090 w=25096,31297,0,0,25097,-1,25094
W3 "".length()=-1
```

`a1` → `a2` is exactly `positivePrefix = "";` in `applyPattern` — a `putfield`
of a reference, which fires the GC write barrier. The barrier pushes the stored
value onto the grey list, and `-1` is `GC.GREY_END`.

**The push should have rejected the literal and does not.** The gate is

```java
if (ref == 0) return;
if (ref < memStart || ref >= hEnd) return;      // 25090 < 35144 -- should return
if ((ref & 0x7) != 0) return;
...
...
Measured on this build: `mem_start = 35144`, `handleEnd = 83104`, and the
literal sits at 25090 — **below `mem_start`, so the range check should have
returned**. It did not. Literals are being pushed onto the grey list today.

**Why nobody has ever seen it.** The push is gated on
`rdMem(ref+OFF_GREY) == 0`, and `OFF_GREY` is offset **+5**. Under the OLD
literal layout, +5 is a character word or the next literal's first word —
non-zero garbage, so the gate stays shut and the barrier silently does nothing.
Verified directly: with the old layout the same probe shows `+5 = 24853`,
unchanged across the whole constructor, and `"".length()` stays 0.

Item 136's fix puts the char[]'s **length** at +5. For every non-empty literal
that is non-zero and nothing changes — but for `""` it is a legitimate **0**,
the gate reads "not on the worklist", and the barrier writes `-1` over the
length. `"".length()` becomes -1, `StringBuffer.append` uses it as a copy count,
and the JVM dies. That is why only `DecimalFormat` broke: `positivePrefix` and
`positiveSuffix` are `""`.

**BOTH FIXED 2026-09-03, in this order:**

1. **[Item 141](#item-141)** — `JVM.f_putfield_ref`/`f_putstatic_ref`
   open-coded the grey-list push without the handle-range screen. Fixed first,
   because item 136 cannot land on top of it.
2. **This item** — `STR_OBJ_LEN` `2+2+1` → `4+2+1`, unpacking the String handle
   so word +3 is a real `OFF_TYPE = IS_OBJ`. The char[] handle stays 2 words;
   widening it as well was tried and is not needed.

**Acceptance:** `StringLiteral` runs with `strict = true` — all eight cases hard
failures — and `DoAll` is **68/68, 136 ok lines over two passes, zero
failures**, with `TextFormatTest ok`. The only MISS remaining in the suite is
`arraylength-NPE`, which is [item 129](#item-129) and unrelated.

**What this cost, and what it bought.** Six wrong hypotheses at 13 minutes each
before the reproducer existed; four sharp 2-minute experiments after. The
reproducer, the word dump and the old-layout control are what turned "a test
fails" into a one-statement root cause. Build it first next time.

**Fix shape — undecided, and the choice matters.** Either give the literal a
real `OFF_TYPE` (costs a word per literal in the image, and moves the `value`
field, which the microcode and `StringInfo` both encode), or make
`f_checkcast`/`f_instanceof` recognise a literal without reading `OFF_TYPE`.
**Write the failing test first** — a `jvm/StringLiteral.java` covering the eight
cases above, with the heap-String control, added to `DoAll`.
...
- Linker: `JopMethodInfo.java:156-161` — `word1 = codeAddress<<10 | len`, and
  `len` is zeroed (the "interpret me" signal) only when
  `len >= JOPizer.METHOD_MAX_SIZE/4` = **512** words.
- Runtime: `Startup.java:283` — `if (len < 256 && len != 0) Native.invoke(addr);
  else interpret();`, with the comment *"see JOPizer constant on max. method
  length"*. **256 ≠ 512.**

`Startup.interpret()` implements ~30 opcodes — constants, `ldc`, `*astore`,
`dup`, `newarray`, `anewarray`, `arraylength`, `get/putstatic`, `return`. No
arithmetic, no branches, no invoke, no `new`, no `getfield`. Anything else hits
`default:`, prints, and **spins forever** (`Startup.java:426-431`).

So a `<clinit>` between 1024 and 2047 bytes is handed to an interpreter that
cannot run it, while being perfectly invokable by hardware (the method cache
limit is 2048 B). Reproduced:

```
len=386 words (invokable):  "CI" then "JVM interpreter: bytecode 184 not implemented" -> hangs
len=234 words (control):    "CI OK M0 y=42 DONE JVM exit!"
```

**Consequence.** Any class whose static initialiser compiles into that band and
is not a pure constant-array fill bricks the board at boot, before `main()`,
with a message naming a bytecode rather than a class. Not hypothetical: `DoAll`
already contains 339- and 303-word methods (`BigInteger.ensureConstants`,
`BigDecimal$StringBuilderHelper.ensureDigitArrays`) whose bodies are static
initialisers in stock OpenJDK.

**Nothing would catch it:** all 18 `<clinit>`s in `DoAll.jop` are ≤ 55 words, so
the interpreter path is never executed by any test.

**Related, same function:** `JopMethodInfo.java:104-114` waives `len >= 512`,
`mreallocals > 31` AND `margs > 31` for `<clinit>`, but only `len` has a
downstream representation. `word2 = cpoolAddress<<10 | mreallocals<<5 | margs`
has a 5-bit locals field, so a `<clinit>` with ≥32 real locals overflows into
the constant-pool address — every `ldc` in that initialiser then resolves one
slot off. Worst method in the image today has 29. Nothing checks it.

**Also blocking:** `make -C java sim-smallest` and `sim-small` cannot run at all
— `JopSim.java:65` caps `MAX_MEM` at 1 MB while `Startup.java:88` falls back to
`appEnd + 262144`, so every app aborts with `writeMem: wrong address`. Raising
it made both reproductions above take about two minutes. Fixing it in-tree gives
the project a working layout test bed that needs no hardware.

> **FIXED 2026-09-04 (`a6d428b`); the leftovers removed 2026-09-05.**
>
> `Startup.clazzinit` now ALWAYS calls `Native.invoke(addr)`. JOPizer applies
...
> helpers (`readBC8u`, `xastore`, `newarray`, …) with no remaining callers, a
> `static int[] stack` of `MAX_STACK = 100` allocated on **every boot** in
> `clazzinit`, and a method-struct decode whose every output — `len`, `locals`,
> `args`, `cp` — went unused once the decision became "always invoke". All of
> it was still being LINKED: removing it took `DoAll.jop` from 2,955,048 to
> **2,947,264 bytes**, 7,784 bytes out of every image on every board.
>
> The class javadoc still advertised "very simple JVM interpreter for large
> `<clinit>` methods" — the documentation outliving the code by a day, which is
> how the item came to look open.
>
> **Validated:** `sbt test` 666/666; `JopJvmTestsBramSim` 136 ok, 0 MISS, with
> `Clinit ok` / `Clinit2 ok`; DoAll **68/68 on EP4CGX150, Wukong and Colorlight
> i5**. RTL unchanged, so the existing bitstreams were reused and only the
> images reloaded.


<a id="item-138"></a>

### Item 138 — the class-struct layout is restated in four languages and one field has no name at all

**Found 2026-09-02** by boundary review B6. Ground truth taken from a GENERATED
artefact first (`HelloWorld.jop:10471-10479`): header is 5 words —
`0 instSize, 1 staticValueVarAddr, 2 instGCinfo, 3 super, 4 iftab`.

The findings, worst first:

- **Word 1 has no named constant anywhere.** `JVMHelp.java:312-313` and
  `:321-322` read it as `Native.rdMem(p+1)` to find a class's static-field
  block. `Const.java` names words 0, 3 and 4 (`CLASS_SIZE`, `CLASS_SUPER`,
  `CLASS_IFTAB`) and `ClassStructConstants.java` names none of them — **word 1
  is absent from both.** A header reorder driven by the named constants leaves
  `rdMem(p+1)` reading `instGCinfo`, and `makeHWObject` then writes the UART I/O
  address into the class-info table. Symptom: a dead board with no output at
  all, because the corruption happens before the first character.
- **`ClassStructConstants.CLS_HEAD` is not the source of what JOPizer emits.**
  `JopClassInfo.java:350` reserves `CLS_HEAD` words; `:796-841` emits exactly
  five with five hardcoded `println`s and no reference to it. Set `CLS_HEAD = 6`
  and every later address drifts one word per class. `JopWriter.java:88-92`
  guards the special-pointer block's size in exactly this way — the guard
  pattern the project already uses is simply absent here.
- **`CLASS_HEADR` (Scala) and `CLS_HEAD` (Java tools) have no link.** An
  exhaustive `command grep` over `*.java *.scala *.asm *.inc *.v *.vhd` finds
  definitions and uses only: no test, no assertion, no Makefile rule mentions
  both.
- **`JopSim` restates `CLASS_SUPER` and `CLASS_HEADR` as literals** at
  `JopSim.java:327-329` while importing the file that defines them, and nothing
  executes those lines — the only entry points run `HelloWorld`, which has no
  non-`<init>` super call. JopSim is what people reach for when hardware and
  simulation disagree, so its wrong answer would be about *where the bug is*.
- **The special-pointer block's SIZE is guarded, its ORDER is not.** Five
  consumers hold raw indices (`jvm.asm:1110-1128`, `Startup.java:117,125`,
  `:92-93`, `JVMHelp.java:224-225`). Swapping slots 4 and 6 passes
  `JopWriter`'s guard and makes `GC.init` treat a class-info address as the
  static-reference root base.
- **`JOPizer.java:61-66` is six stale constants**, one already self-inconsistent
  (`CLASSINFO_REFARRY` = 7, the same as `CLASSINFO_NONREFARRY`, where the
  pattern of the neighbouring lines gives 9), and the single apparent use passes
  a value into a parameter `StringInfo.dump` never references.
- **`docs/architecture/constant-dependencies.md:302-305` points at the wrong
  file** — it attributes `MTAB2CLINFO`, `MTAB2GC_INFO`, `IMPORTANT_PTRS` and
  `CLINITS_OFFSET` to `ClassStructConstants.java`, which defines exactly two
  constants and none of those. Following the doc leads you to edit the half with
  no effect on the runtime.
- **`Const.RAM_CP = 1` restates the declaration ORDER of `?` variables in
  `jvm.asm`**, where a comment (*"keep order! these vars are accessed from Java
  progs"*) is the only defence. `Jopa.java:237,299` assigns addresses purely by
  source order. `Const.java` has no Make dependency on `asm/src/*`.

**Genuinely clean, and worth recording as such:** the interface bit-table
indexing agrees end to end across four independent restatements; the instance
GC-info bit order agrees and `instSize > 31` is a hard error; the RTL knows
nothing of the class struct at all (so `constant-dependencies.md:296`'s claim
that `METH_STR` is "also in Pipeline method dispatch" is wrong); and
...
### ~~Item 140~~ — `make -C java/apps/<X>` did not rebuild JOPizer, so a LINKER change silently did not take effect — **FIXED 2026-09-04**

**Found 2026-09-03** while bisecting [item 136](#item-136), and it produced an
impossible result rather than a wrong one, which is the only reason it was
caught.

`make -C java runtime && make -C java/apps/JvmTests` rebuilds the runtime and
relinks the application. It does **not** rebuild `java/tools/dist/jopizer.jar`.
So a change to the linker — `StringInfo.java`, `JopClassInfo.java`, anything
under `java/tools/src/com/jopdesign/build/` — is compiled into nothing, every
timestamp looks fresh, and the build exits 0.

The observed shape: `StringInfo.java` was reverted to the unfixed layout, the
app was rebuilt without `make -C java tools`, and the run reported
`StringLiteral ok` **with no MISS lines** — a result that cannot happen with the
defect present. The image told the truth where the source did not:

```
$ command grep -c 'OFF_TYPE: IS_OBJ' java/apps/JvmTests/DoAll.jop
374                      # from a jopizer.jar built 40 minutes earlier
$ command grep -n 'STR_OBJ_LEN =' .../StringInfo.java
37:     final static int STR_OBJ_LEN = 2+2+1;      # the source says otherwise
```

**This is a DIFFERENT trap from the one already recorded.** The known GOTCHA is
that `make -C java all` does not reliably rebuild apps, giving a stale `.jop`;
its fix is `make -C java runtime && make -C java/apps/<X> clean && make ...`,
which is exactly the incantation used here — and it is not enough, because the
staleness is one level further up, in the tool that produces the `.jop`.

**Why it is dangerous rather than annoying.** A linker change that appears to
work is how a false conclusion gets committed. Had the stale jar happened to
agree with the source, nothing would have looked wrong. Four other runs in the
same bisect did include `make -C java tools` and their results stand — that was
luck of habit, not a property of the build.

**FIXED 2026-09-04, and the structure turned out to be simpler than assumed.**

Looking before prescribing changed the fix. The app Makefiles have **no file
targets at all** — `classes:`, `preprocess: classes` and `jop: preprocess` are
phony-style and always run — so the app itself never goes stale. The staleness
was entirely in the jar, which lives in another directory and was never invoked
or checked: the app simply ran JOPizer from whatever was on disk.

`java/tools/dist/jopizer.jar` is already a proper file target over
`find $(SRC_DIR) -name '*.java'`, so the sub-make is incremental. That makes the
fix a single line rather than a dependency graph: `java/config.mk` — which all
eight apps include — declares
...
A rule with no recipe only ADDS a prerequisite to the `jop` target each app
defines later, so one declaration covers all eight.

**Measured, because "always rebuild the linker" sounds expensive and is not:**

| case | cost |
|---|---|
| jar up to date | **28 ms** — a stat and "nothing to be done" |
| after touching a linker source | 1620 ms |

**Red before green, through the documented workflow.** A marker was added to
`StringInfo.dump`, then `make -C java runtime && make -C java/apps/Smallest
clean && make ...` — the incantation the old GOTCHA prescribes:

```
before: marker in emitted .jop: 0     <- the change did not reach the image
after:  marker in emitted .jop: 67
```

**One trap on the way, worth recording.** The first version made `tools-fresh`
the DEFAULT GOAL for every Makefile including `config.mk` — a rule defined in an
include becomes the default if it is the first target make sees, and
`java/runtime` includes this file too, so `make -C java runtime` started trying
to build the tools with `TOOLS_DIR` unset. Fixed with `.DEFAULT_GOAL :=` after
the block, which hands the choice back to the including file's own first target.

**Guarded** in `check-generated-deps.sh`. The obvious check — `make -pn jop |
grep tools-fresh` — reads the real rule database but still executes sub-makes,
so it recursed into `java/tools`, emitted a 200-line dump, and gave an
ORDER-DEPENDENT answer: it reported PerfTest failing on one run and passing on
the next with the edge present both times. A flaky guard is worse than none, so
it asserts the three facts that produce the edge instead — the declaration in
`config.mk`, each app's include, each app's `jop` target — which is
deterministic and needs no build. Proved red by commenting out the declaration,
and again by removing one app's include.


<a id="item-141"></a>

### ~~Item 141~~ — the GC write barrier pushed references BELOW `mem_start` onto the grey list — **FIXED 2026-09-03**

**Found 2026-09-03** while root-causing [item 136](#item-136). Live on every
board; invisible only by accident.

A `putfield` of a reference fires the write barrier, which pushes the stored
value onto the grey list. The push is guarded:

```java
if (ref < memStart || ref >= hEnd) return;
```

Measured on a running system: `mem_start = 35144`, `handleEnd = 83104`, and a
String literal at **25090** — comfortably below `mem_start`. The guard should
...
**Why it has never been noticed.** The final gate is
`if (rdMem(ref+OFF_GREY) == 0)`. With today's literal layout, offset +5 holds a
character or the next literal's first word — non-zero — so the gate stays shut
and the barrier is a silent no-op on literals. Confirmed by the control: same
probe, old layout, `+5` never changes.

**Why it matters even so.** The barrier is writing into the read-only string
table whenever that gate happens to open. Any change to the literal layout, any
literal whose +5 word is legitimately zero, or any other below-heap reference
with a zero at +5, turns a no-op into memory corruption of the program image.
...
if (oldVal != 0
    ...
    Native.wrMem(GC.grayList, oldVal+GC.OFF_GREY);   // no bounds, no alignment
    GC.grayList = oldVal;
}
```

`GC.java:146` states the discipline — *"push() and pushYoung() screen every
candidate root against the handle area"* — and these two sites predate or
sidestep it. Every reference store whose OLD value is a String literal reaches
that write.

**Fix:** the range and alignment checks inline, at both sites. It deliberately
does NOT delegate to `GC.push()`: that takes `GC.mutex`, which is already held
here, and the global lock is **not reentrant** — the trap that broke the SMP GC
for days.
